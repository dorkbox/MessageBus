...
public class StampedLock implements java.io.Serializable {
    ...
    static final class WNode {
        ...
        WNode(int m, WNode p) { this.mode = m; this.prev = p; }
    }
    ...
    public long readLock() {
        long s = this.state, next;  // bypass acquireRead on common uncontended case
        return this.whead == this.wtail && (s & ABITS) < RFULL &&
    ...
    public long tryOptimisticRead() {
        long s;
        return ((s = this.state) & WBIT) == 0L ? s & SBITS : 0L;
    }
    ...
    public void unlockWrite(long stamp) {
        WNode h;
        if (this.state != stamp || (stamp & WBIT) == 0L) {
            throw new IllegalMonitorStateException();
        }
        this.state = (stamp += WBIT) == 0L ? ORIGIN : stamp;
    ...
    public void unlockRead(long stamp) {
        long s, m; WNode h;
        for (;;) {
            if (((s = this.state) & SBITS) != (stamp & SBITS) ||
                (stamp & ABITS) == 0L || (m = s & ABITS) == 0L || m == WBIT) {
                throw new IllegalMonitorStateException();
            }
            if (m < RFULL) {
                if (U.compareAndSwapLong(this, STATE, s, s - RUNIT)) {
                    if (m == RUNIT && (h = this.whead) != null && h.status != 0) {
                        release(h);
                    }
                    break;
                }
    ...
    public void unlock(long stamp) {
        ...
        while (((s = this.state) & SBITS) == (stamp & SBITS)) {
            if ((m = s & ABITS) == 0L) {
                break;
            } else if (m == WBIT) {
                if (a != m) {
    ...
    public long tryConvertToOptimisticRead(long stamp) {
        ...
        for (;;) {
            ...
            if ((m = s & ABITS) == 0L) {
                ...
                this.state = next = (s += WBIT) == 0L ? ORIGIN : s;
                if ((h = this.whead) != null && h.status != 0) {
                    release(h);
                }
                return next;
    ...
    public boolean tryUnlockRead() {
        long s, m; WNode h;
        while ((m = (s = this.state) & ABITS) != 0L && m < WBIT) {
            if (m < RFULL) {
                if (U.compareAndSwapLong(this, STATE, s, s - RUNIT)) {
                    if (m == RUNIT && (h = this.whead) != null && h.status != 0) {
                        release(h);
    ...
    private int getReadLockCount(long s) {
        ...
        if ((readers = s & RBITS) >= RFULL) {
            readers = RFULL + this.readerOverflow;
        }
        return (int) readers;
    }
    ...
    public Lock asReadLock() {
        ReadLockView v;
        return (v = this.readLockView) != null ? v :
    ...
    public Lock asWriteLock() {
        WriteLockView v;
        return (v = this.writeLockView) != null ? v :
    ...
    public ReadWriteLock asReadWriteLock() {
        ReadWriteLockView v;
        return (v = this.readWriteLockView) != null ? v :
    ...
    final void unstampedUnlockRead() {
        for (;;) {
            long s, m; WNode h;
            if ((m = (s = this.state) & ABITS) == 0L || m >= WBIT) {
                throw new IllegalMonitorStateException();
            } else if (m < RFULL) {
                if (U.compareAndSwapLong(this, STATE, s, s - RUNIT)) {
                    if (m == RUNIT && (h = this.whead) != null && h.status != 0) {
    ...
    private void readObject(java.io.ObjectInputStream s)
        ...
        s.defaultReadObject();
        this.state = ORIGIN; // reset to unlocked state
    }
    ...
    private long acquireWrite(boolean interruptible, long deadline) {
        ...
        for (int spins = -1;;) { // spin while enqueuing
            ...
            else if ((p = this.wtail) == null) { // initialize queue
        ...
        for (int spins = -1;;) {
            ...
            if (this.whead == h) {
                if ((np = node.prev) != p) {
                    ...
                    node.thread = wt;
                    if (p.status < 0 && (p != h || (this.state & ABITS) != 0L) &&
                        this.whead == h && node.prev == p)
                     {
    ...
    private long acquireRead(boolean interruptible, long deadline) {
        ...
        for (int spins = -1;;) {
                ...
                for (long m, s, ns;;) {
                    if ((m = (s = this.state) & ABITS) < RFULL ?
                        ...
                        if (spins > 0) {
                            ...
                            if (spins == 0) {
                                WNode nh = this.whead, np = this.wtail;
            ...
            if (p == null) { // initialize queue
                ...
                for (;;) {
                    WNode pp, c; Thread w;
                    if ((h = this.whead) != null && (c = h.cowait) != null &&
                    ...
                    if (this.whead == h && p.prev == pp) {
                        ...
                        U.putObject(wt, PARKBLOCKER, this);
                        node.thread = wt;
                        if ((h != pp || (this.state & ABITS) == WBIT) &&
                            this.whead == h && p.prev == pp) {
                            U.park(false, time);
                        }
                        node.thread = null;
                        U.putObject(wt, PARKBLOCKER, null);
        ...
        while ((h = this.whead) != null) {
            ...
            if (h == this.whead) {
                if (q != null && h.status == 0 &&
                    ((s = this.state) & ABITS) != WBIT && // waiter is eligible
                    (s == 0L || q.mode == RMODE)) {
                    release(h);
                }
                break;
